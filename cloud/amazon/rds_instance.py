...
class RDSDBInstance(object):
    ...
    def __eq__(self, other):
        ...
        compare_keys = ['availability_zone', 'backup_retention', 'id', 'instance_type', 'iops',
                         'maintenance_window', 'multi_zone',
                        'replication_source', 'username',
                        'vpc_security_groups', 'endpoint', 'port']
        return all([data.get(k) == otherdata.get(k) for k in compare_keys])
...
def create_db_instance(module, conn):
    ...
    module.exit_json(changed=changed, instance=resource.get_data(), operation="create")
...
def replicate_db_instance(module, conn):
    ...
    valid_vars = ['instance_type', 'iops', 'option_group', 'port',
                  'publicly_accessible', 'tags', 'upgrade', 'zone']
    params = validate_parameters(required_vars, valid_vars, module)
    instance_name = module.params.get('instance_name')
    ...
    module.exit_json(changed=changed, instance=resource.get_data(), operation="replicate")
...
def modify_db_instance(module, conn):
    ...
    params = validate_parameters(required_vars, valid_vars, module)
    old_instance_name = module.params.get('old_instance_name')
    instance_name = module.params.get('instance_name')
    old_instance = get_db_instance(conn, old_instance_name)
    ...
    if params.get('apply_immediately'):
        if old_instance_name:
            ...
            while not new_instance:
                new_instance = get_db_instance(conn, instance_name)
                time.sleep(5)
    ...
    changed = (instance == old_instance)
    module.exit_json(changed=changed, instance=instance.get_data(), operation="modify")
...
def promote_db_instance(module, conn):
    ...
    module.exit_json(changed=changed, instance=instance.get_data(), operation="promote")
...
def reboot_db_instance(module, conn):
    ...
    module.exit_json(changed=True, instance=instance.get_data(), operation="reboot")
...
def restore_db_instance(module, conn):
    ...
    module.exit_json(changed=changed, instance=instance.get_data(), operation="restore")
...
def ensure_db_state(module, conn):
    ...
    if instance.get_data().get('replication_source') and not module.params.get('source_instance'):
        promote_db_instance(module, conn)
    modify_db_instance(module, conn)
    module.exit_json(changed=False, instance=instance.get_data())
